...
- Relinked to the library with every stored modifier value carried over, including attribute bindings (`_use_attribute`/`_attribute_name`):
  ...
  - local `GN_Push` → `GN_Inflate` on Cube.002/.003/.004/.005 (for Cube.004 see below).
- Every GN_ modifier is now named after its node group (Blender adds .001 for duplicates on one object).
...
- Cube.004's Push (-0.01, off in the viewport) is now GN_Inflate as well (2026-09-25). Same selection (`PushIn` attribute) and the same 370 vertices move. Push averaged the face normals without renormalizing, so it moved 0.009 on average; Inflate moves exactly 0.01. 183 vertices differ by up to 0.0047. Accepted.
