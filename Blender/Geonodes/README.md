...
| — | **GN_CurveFromSurface** | `GN_CurveFromSurface.blend` | *Work in progress, taken from the character example.* Runs a tube along the mesh's open borders or an edge selection — trims, piping, seams — with offset, inset, a circle / spiral / custom profile, tilt and rim crease | Selection, Keep Geometry, Auto Border from Surface, Edge Selection, Offset, Inset, Profile Shape, Base Profile Radius, Crease RimLoop, Material |
| — | **GN_SpawnObjectsAlong** | `GN_SpawnObjectsAlong.blend` | *Work in progress, taken from the character example.* Turns edges into curves and either sweeps a profile along them or repeats and bends an object along each curve, with UVs — zipper teeth, stitches, chains. Needs third-party node groups for now (see the tool notes) | Type, Radius, Scale, Edge Group, Surface SubDiv Level, Offset, Object, Rotation, Tilt, Material |
| — | **GN_DistributeObjectOnSurface** | `GN_DistributeObjectOnSurface.blend` | *Work in progress, taken from the character example.* Places an object on every vertex, edge, face or corner of a selection — studs, rivets, buttons — optionally every nth element, on a grown / shrunk selection, or on a subdivided surface | Selection, Grow Iterations, Self Surface, Keep Surface, Subdiv Level, Distribution Mode, nth Selection Offset, Distribution Object, Material |
