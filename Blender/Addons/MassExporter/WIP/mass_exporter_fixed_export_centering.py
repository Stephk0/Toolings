bl_info = {
    "name": "Mass Collection Exporter - FIXED Export Centering",
    "author": "Claude AI", 
    "version": (5, 0, 0),
    "blender": (3, 0, 0),
    "location": "3D View > N-Panel > Mass Exporter",
    "description": "Export collections with PROPERLY WORKING empty centering during export",
...
def move_empty_to_origin_simple(empty, debug_mode=False):
    """
    Move an empty to origin using the simple, reliable method.
    ...
    # Simple, direct approach - same as empty_center_tool.py
...
def restore_empty_location_simple(empty, original_location, debug_mode=False):
    """
    Restore an empty to its original location.
    """
    if debug_mode:
...
class MASSEXPORTER_OT_move_empties_to_origin(Operator):
    """Move all empties with children to origin and leave them there"""
    bl_idname = "massexporter.move_empties_to_origin"
    bl_label = "Move Empties to Origin"
    bl_description = "Move all empties with mesh children to origin (0,0,0) and leave them there"
    ...
    def execute(self, context):
        ...
        for obj in bpy.context.scene.objects:
            if obj.type == 'EMPTY':
                ...
                if children:
                    print(f"Moving {obj.name} from {obj.location} to origin")
        ...
        if empties_moved:
            self.report({'INFO'}, f"Moved {len(empties_moved)} empties to origin")
            print(f"\nSUCCESS: Moved {len(empties_moved)} empties to origin (0,0,0)")
            print("Use Ctrl+Z to undo if needed")
        else:
            self.report({'WARNING'}, "No empties with mesh children found")
            print("No empties with mesh children found in scene")
...
class MASSEXPORTER_OT_export_all(Operator):
    ...
    def export_with_empty_origins(self, context, props, item):
        ...
        if empties_with_children:
            ...
            # STEP 1: Move ALL empties to origin FIRST (like the manual button)
            if item.center_parent_empties:
                if props.debug_mode:
                    print("\n--- MOVING ALL EMPTIES TO ORIGIN BEFORE EXPORT ---")
                
                for empty, children in empties_with_children.items():
                    if props.debug_mode:
                        print(f"Moving {empty.name} from {empty.location} to origin")
                    
                    # Use the same method as the manual button
                    empty.location = (0.0, 0.0, 0.0)
                    bpy.context.view_layer.update()
            ...
            # STEP 2: Export each empty group (while they're all at origin)
            for empty, children in empties_with_children.items():
                if props.debug_mode:
                    print(f"\n--- EXPORTING: {empty.name} with {len(children)} children ---")

                if item.combine_empty_children:
                    # Combine children into single mesh before export
                    if self.export_combined_empty_children_fixed(context, props, children, empty, export_path, item):
                        success_count += 1
                else:
                    # Export each child individually (empties already at origin)
                    for child in children:
                        if self.export_single_object_at_origin(context, props, child, child.name, export_path):
                            success_count += 1
    ...
    def export_combined_empty_children_fixed(self, context, props, children, empty, export_path, item):
        """Combine children into single mesh and export - FIXED VERSION"""
        if not children:
            return False
        ...
        if props.debug_mode:
            print(f"Combining and exporting children of {empty_name}")
        ...
        try:
            # Combine children into single mesh (if more than one)
            if len(children) > 1:
                if props.debug_mode:
                ...
                result = self.export_single_object_at_origin(context, props, combined_obj, f"{empty_name}_combined", export_path)

                # Undo the join operation to restore individual objects
                bpy.ops.ed.undo()

            else:
                # Only one child, just export it
                child = children[0]
                if props.debug_mode:
                    print(f"Single child, exporting as: {empty_name}")

                result = self.export_single_object_at_origin(context, props, child, empty_name, export_path)
    ...
    def export_single_object_at_origin(self, context, props, obj, name, export_path):
        """Export a single object that's already positioned correctly"""
        if props.debug_mode:
...
class MASSEXPORTER_PT_main_panel(Panel):
    ...
    bl_label = "Mass Exporter - FIXED Export Centering"
    ...
    def draw(self, context):
        ...
        # Info text
        centering_box.label(text="↑ Moves empties to (0,0,0) and leaves them there", icon='INFO')
        
        # Status info
        status_box = layout.box()
        status_box.label(text="FIXED: Export now uses same centering method", icon='CHECKMARK')
        status_box.label(text="Objects will export at origin correctly")
    ...
    def draw(self, context):
        ...
        if props.collection_items and len(props.collection_items) > props.active_collection_index:
            ...
            if active_item.use_empty_origins:
                ...
                # Add note about fixed approach
                note_box = box.box()
                note_box.label(text="FIXED: Export centering works correctly", icon='CHECKMARK')
                note_box.label(text="Uses same method as manual button")
