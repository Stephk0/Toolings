bl_info = {
    ...
    "description": "Export collections with auto-move empties to origin and join empty children features - COPY-BASED JOIN",
...
def join_empty_children_core_logic(context, report_func=None, apply_modifiers=False, keep_joined_copy=True):
    ...
    try:
        ...
        for empty_name, (empty, children, parent_collection) in unique_empties.items():
            ...
            if joined_obj:
                ...
                print(f"  Successfully joined {len(duplicates)} duplicates into '{joined_obj.name}'")
    ...
    if joined_empties:
        ...
        print(f"\nSUCCESS: Created joined copies for {len(joined_empties)} empties")
        if keep_joined_copy:
...
class MASSEXPORTER_OT_export_all(Operator):
    ...
    def execute(self, context):
        ...
        # Track joined objects created during export for cleanup
        temp_joined_objects = []
        ...
        # Create temporary joined copies (keep_joined_copy=False)
        if join_collections:
            if props.debug_mode:
            ...
            # Use the copy-based core logic function (don't keep copies - they're temporary for export)
            result = join_empty_children_core_logic(context, self.report, apply_modifiers, keep_joined_copy=False)
            ...
            if result == {'CANCELLED'}:
                ...
                # Track the joined objects for cleanup later
                for obj in bpy.data.objects:
                    if obj.type == 'MESH' and obj.name.endswith('_joined') and obj.hide_viewport:
                        temp_joined_objects.append(obj)
                        if props.debug_mode:
                            print(f"Tracking temporary joined object: {obj.name}")
        ...
        try:
            ...
            # STEP 4: Clean up temporary joined objects
            if temp_joined_objects:
                if props.debug_mode:
                    print(f"\n=== CLEANING UP {len(temp_joined_objects)} TEMPORARY JOINED OBJECTS ===")
                
                for obj in temp_joined_objects:
                    if obj.name in bpy.data.objects:
                        if props.debug_mode:
                            print(f"Deleting temporary object: {obj.name}")
            ...
            if item.center_parent_empties:
                ...
                try:
                    ...
                    if item.join_empty_children:
                        # Look for already created joined copy
                        if self.export_children_combined(context, props, empty, children, export_path, item):
                            success_count += 1
                    else:
                        # Export individually
    ...
    def export_children_combined(self, context, props, empty, children, export_path, item):
        """Export children combined - looks for already created joined copy and ONLY exports that"""
        if not children:
            return False
        ...
        # Look for already joined object (created by join step)
        # Try multiple naming patterns based on collection name
        collection_name = item.collection.name
        possible_names = [
            f"{collection_name}_joined",
            f"{empty_name}_joined",
        ]
        
        joined_obj = None
        for name in possible_names:
            if name in bpy.data.objects:
                joined_obj = bpy.data.objects[name]
                break
            # Also try numbered versions
            for i in range(1, 10):
                numbered_name = f"{name}_{i}"
                if numbered_name in bpy.data.objects:
                    joined_obj = bpy.data.objects[numbered_name]
                    break
            if joined_obj:
                break
        
        if joined_obj:
            # Use the already joined object - ONLY export this, not the children
            if props.debug_mode:
                print(f"Found already joined object: {joined_obj.name}")
                print(f"ONLY exporting joined object, skipping original children")
            
            # Unhide it temporarily if hidden
            was_hidden = joined_obj.hide_viewport
            if was_hidden:
                joined_obj.hide_set(False)
                joined_obj.hide_viewport = False
            
            # Export ONLY the joined object
            result = self.export_single_object_simple(context, props, joined_obj, empty_name, export_path)
            
            # Delete the joined object after export
            if props.debug_mode:
                print(f"Deleting joined object after export: {joined_obj.name}")
            bpy.data.objects.remove(joined_obj, do_unlink=True)
            
            return result
        else:
            # Fall back: export children individually
            if props.debug_mode:
                print(f"No joined object found for {empty_name}, exporting children individually")
            return self.export_children_individual(context, props, empty, children, export_path)
...
class MASSEXPORTER_PT_main_panel(Panel):
    ...
    bl_label = "Mass Collection Exporter (COPY-JOIN)"
    ...
    def draw(self, context):
        ...
        row = controls_box.row()
        row.scale_y = 1.2
        row.operator("massexporter.join_empties", text="Join Empties (Create Copies)", icon='MOD_SOLIDIFY')
        
        controls_box.label(text="Creates joined copies without modifying originals")
        ...
        if props.collection_items and len(props.collection_items) > props.active_collection_index:
            ...
            if active_item.use_empty_origins:
                ...
                if active_item.join_empty_children:
                    ...
                    mod_box.label(text="ONLY joined object is exported")
                    mod_box.label(text="Joined object deleted after export")
