bl_info = {
    ...
    "author": "Claude AI", 
    "version": (9, 0, 0),  # UPDATED VERSION - ON-DEMAND JOIN
    "blender": (3, 0, 0),
    "location": "3D View > N-Panel > Mass Exporter",
    "description": "Export collections with on-demand join during export - simpler and more reliable",
...
# NEW COPY-BASED JOIN FUNCTION (for debug button only)
...
class CollectionExportItem(PropertyGroup):
    ...
    join_empty_children: BoolProperty(
        ...
        description="Join all mesh children of empties into single objects during export (on-demand)",
        default=False
    )
...
class MASSEXPORTER_OT_export_all(Operator):
    ...
    def execute(self, context):
        ...
        # ========== REMOVED PRE-JOIN STEP ==========
        # Join now happens on-demand during export in export_children_combined
            ...
            if item.center_parent_empties:
                ...
                try:
                    ...
                    if item.join_empty_children:
                        # Join on-demand and export
                        if self.export_children_combined(context, props, empty, children, export_path, item, collection.name):
                            success_count += 1
                    else:
                        # Export individually
    ...
    def export_children_combined(self, context, props, empty, children, export_path, item, collection_name):
        """
        ========== FIXED VERSION - ON-DEMAND JOIN ==========
        Creates joined copy on-the-fly, names after collection, exports, and deletes immediately
        """
        if not children or len(children) < 2:
            if props.debug_mode:
                print(f"Skipping join for {empty.name} - only {len(children)} child(ren)")
            # Fall back to individual export
        ...
        if props.debug_mode:
            print(f"\n=== ON-DEMAND JOIN for {empty.name} ===")
            print(f"Children to join: {[child.name for child in children]}")
        
        try:
            empty_world_loc = empty.location.copy()
            
            # STEP 1: Duplicate children
            if props.debug_mode:
                print("Duplicating children...")
            bpy.ops.object.select_all(action='DESELECT')
            for child in children:
                ...
                for dup in duplicates:
                    ...
                    for mod in dup.modifiers[:]:
                        try:
                            ...
                            if props.debug_mode:
                                print(f"  Applied modifier '{mod.name}' to {dup.name}")
                        except Exception as e:
                            if props.debug_mode:
                                print(f"  Failed to apply modifier '{mod.name}': {str(e)}")
            
            # STEP 3: Deparent duplicates
            if props.debug_mode:
                print("Deparenting duplicates...")
            for dup in duplicates:
                if dup.parent:
            ...
            # STEP 5: Get and rename joined object (EXACTLY collection name)
            joined_obj = context.active_object
            if joined_obj:
                # KEY FIX: Name exactly after collection (not {collection}_joined)
                joined_obj.name = collection_name
                
                if props.debug_mode:
                    print(f"Renamed joined object to: {joined_obj.name}")
                
                # STEP 6: Set origin to empty location
                if props.debug_mode:
                    print(f"Setting origin to empty location: {empty_world_loc}")
                ...
                if props.debug_mode:
                    ...
                    print(f"Final joined object world pos: {joined_obj.matrix_world.translation}")
                
                # STEP 7: Export ONLY the joined object
                if props.debug_mode:
                    print(f"Exporting joined object as '{collection_name}'...")
                result = self.export_single_object_simple(context, props, joined_obj, collection_name, export_path)
            ...
            self.report({'ERROR'}, f"Error during on-demand join: {str(e)}")
            if props.debug_mode:
                print(f"ERROR during on-demand join: {str(e)}")
            return False
        
        finally:
            # STEP 8: Delete joined object immediately (KEY FIX)
...
class MASSEXPORTER_PT_main_panel(Panel):
    ...
    bl_label = "Mass Collection Exporter (ON-DEMAND)"
    ...
    def draw(self, context):
        ...
        controls_box.label(text="Join during export happens on-demand per empty")
        controls_box.label(text="(no pre-joining step)")
        ...
        if props.collection_items and len(props.collection_items) > props.active_collection_index:
            ...
            if active_item.use_empty_origins:
                ...
                if active_item.join_empty_children:
                    ...
                    mod_box.label(text="Join happens on-demand during export")
                    mod_box.label(text="Named after collection, deleted after export")
                ...
                if active_item.move_empties_to_origin_on_export:
                    ...
                    info_box.label(text="(after any joining is done)")
