bl_info = {
    "name": "Mass Collection Exporter Fixed v2",
    "author": "Claude AI",
    "version": (4, 1, 0),
    "blender": (3, 0, 0),
    "location": "3D View > N-Panel > Mass Exporter",
    "description": "Export collections with fixed parent empty centering and mesh combining - No reference errors",
...
def execute(self, context):
        ...
        maintain_relative_transforms = True
        ...
        try:
            if props.collection_items and len(props.collection_items) > 0:
                ...
                maintain_relative_transforms = bool(getattr(active, "maintain_relative_transforms", True))
        ...
        for empty_name, children_names in empties_data.items():
            if self.test_empty_operations(
                ...
                maintain_relative_transforms=maintain_relative_transforms,
    ...
    def test_empty_operations(self, empty_name, children_names, *, center_parent_empties=True, maintain_relative_transforms=True, combine_empty_children=False):
        """Test centering and combining operations - FIXED to avoid reference errors"""
        ...
        try:
            ...
            if center_parent_empties:
                ...
                if maintain_relative_transforms:
                    for child in children:
            ...
            if combine_empty_children and len(children) > 1:
                ...
                print("\nNo join performed; no undo necessary")
                
                # After undo, get fresh references
                objects_restored = len([obj for obj in bpy.context.scene.objects if obj.type == 'MESH'])
                print(f"Mesh objects after undo: {objects_restored}")
            else:
...
class MASSEXPORTER_OT_export_all(Operator):
    ...
    def export_combined_empty_children(self, context, props, children, empty, export_path, item):
        """FIXED: Actually combine children into single mesh and export"""
        if not children:
            return False
    ...
    def export_objects_with_empty_origin(self, context, props, objects, empty, name, export_path, item):
        ...
        try:
            ...
            # Always restore original transforms - FIXED with name lookups
            if props.debug_mode:
            ...
            if props.debug_mode:
                print(f"Empty restored to: {empty_original_transform['location']}")
...
class MASSEXPORTER_PT_main_panel(Panel):
    ...
    bl_label = "Mass Exporter Fixed v2"
