bl_info = {
    "name": "Mass Collection Exporter - EXACT BUTTON LOGIC",
    "author": "Claude AI", 
    "version": (5, 5, 0),
    "blender": (3, 0, 0),
    "location": "3D View > N-Panel > Mass Exporter",
    "description": "Export collections using EXACT SAME LOGIC as the working 'move empties to origin' button",
...
class CollectionExportItem(PropertyGroup):
    ...
    use_empty_origins: BoolProperty(
        ...
        description="Search for parent empties and use them as origins/centers. Uses EXACT SAME LOGIC as the working button",
        default=False
    )
...
class MASSEXPORTER_OT_export_all(Operator):
    """Export all enabled collections using EXACT SAME LOGIC as the working button"""
    bl_idname = "massexporter.export_all"
    bl_label = "Export All"
    bl_description = "Export all enabled collections using EXACT SAME LOGIC as the working 'move empties to origin' button"
...
class MASSEXPORTER_PT_main_panel(Panel):
    ...
    bl_label = "Mass Exporter - EXACT BUTTON LOGIC"
    ...
    def draw(self, context):
        ...
        centering_box.label(text="Export will use EXACT SAME LOGIC!", icon='LINKED')
        
        # Status info
        status_box = layout.box()
        status_box.label(text="EXACT BUTTON LOGIC: Copy working button behavior", icon='CHECKMARK')
        status_box.label(text="No differences - same code paths used!", icon='PIVOT_INDIVIDUAL')
