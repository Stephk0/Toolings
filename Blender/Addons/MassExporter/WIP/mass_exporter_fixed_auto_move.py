bl_info = {
    "name": "Mass Collection Exporter - FIXED AUTO MOVE",
    "author": "Claude AI", 
    "version": (5, 7, 0),
    "blender": (3, 0, 0),
    "location": "3D View > N-Panel > Mass Exporter",
    "description": "Export collections with FIXED auto-move empties to origin feature",
...
def move_empties_to_origin_core_logic(context, report_func=None):
    ...
    # Move each empty to origin - THIS IS THE WORKING LOGIC
...
class MASSEXPORTER_OT_export_all(Operator):
    """Export all enabled collections with FIXED auto-move empties to origin"""
    bl_idname = "massexporter.export_all"
    bl_label = "Export All"
    bl_description = "Export all enabled collections with FIXED auto-move empties to origin"
    ...
    def execute(self, context):
        ...
        if auto_move_collections:
            ...
            # FIXED: Use the standalone core logic function instead of trying to instantiate operator
...
class MASSEXPORTER_PT_main_panel(Panel):
    ...
    bl_label = "Mass Exporter - FIXED AUTO MOVE"
    ...
    def draw(self, context):
        ...
        centering_box.label(text="FIXED: Can auto-run this on export!", icon='LINKED')
        
        # Status info
        status_box = layout.box()
        status_box.label(text="FIXED: Auto 'Move to Origin' option in collections", icon='CHECKMARK')
    ...
    def draw(self, context):
        ...
        if props.collection_items and len(props.collection_items) > props.active_collection_index:
            ...
            if active_item.use_empty_origins:
                ...
                # FIXED AUTO MOVE FEATURE
                auto_box = box.box()
                auto_box.label(text="Auto Move to Origin (FIXED):", icon='OUTLINER_OB_EMPTY')
                ...
                if active_item.move_empties_to_origin_on_export:
                    ...
                    info_box.label(text="FIXED: Will auto-run 'Move Empties to Origin'", icon='CHECKMARK')
                    info_box.label(text="before export using same core logic function", icon='LINKED')
                
                # Add note about EXACT BUTTON LOGIC
                note_box = box.box()
                note_box.label(text="EXACT BUTTON LOGIC: Same core function!", icon='CHECKMARK')
