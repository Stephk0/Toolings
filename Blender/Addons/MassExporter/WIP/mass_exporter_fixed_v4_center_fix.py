bl_info = {
    "name": "Mass Collection Exporter Fixed v4 - Center Fix",
    "author": "Claude AI",
    "version": (4, 2, 0),
    "blender": (3, 0, 0),
    "location": "3D View > N-Panel > Mass Exporter",
    "description": "Export collections with FIXED parent empty centering - Children automatically follow parents",
...
class CollectionExportItem(PropertyGroup):
    ...
    center_parent_empties: BoolProperty(
        ...
        description="Move parent empties to world center (0,0,0) before export - children will automatically follow",
        default=True
    )
...
class MASSEXPORTER_OT_test_empty_centering(Operator):
    ...
    def test_empty_operations(self, empty_name, children_names, *, center_parent_empties=True, combine_empty_children=False):
        """Test centering and combining operations - FIXED to properly center parent empties"""
        ...
        # Store original transforms as data, not references
        ...
        try:
            # Test 1: Center empty - FIXED VERSION
            print("\n--- Testing Empty Centering (FIXED) ---")
            if center_parent_empties:
                print(f"Moving empty from {empty.location} to (0,0,0)")
                print("Children will automatically follow due to parent-child relationship")
                
                # Simply move the parent empty - children follow automatically
            ...
            if empty:
                # Restore empty location - children will automatically follow
                print(f"Restoring empty to original location: {empty_original_location}")
...
class MASSEXPORTER_OT_export_all(Operator):
    ...
    def export_combined_empty_children(self, context, props, children, empty, export_path, item):
        """FIXED: Actually combine children into single mesh and export with proper centering"""
        if not children:
            return False
        ...
        try:
            ...
            if item.center_parent_empties:
                ...
                # Update scene to reflect parent-child transforms
                bpy.context.view_layer.update()
            ...
            empty = bpy.data.objects.get(empty_name)
            if empty:
                # Restore empty location - children will follow automatically
                empty.location = empty_original_location
                bpy.context.view_layer.update()
    ...
    def export_objects_with_empty_origin(self, context, props, objects, empty, name, export_path, item):
        """Export objects using empty's location as origin - FIXED VERSION with proper centering"""
        if not objects:
            return False
        ...
        try:
            ...
            empty = bpy.data.objects.get(empty_name)
            if empty:
                # Restore empty's original location - children will follow automatically
                empty.location = empty_original_location
                bpy.context.view_layer.update()
...
class MASSEXPORTER_PT_main_panel(Panel):
    ...
    bl_label = "Mass Exporter Fixed v4 - Center Fix"
    ...
    def draw(self, context):
        ...
        if props.collection_items and len(props.collection_items) > props.active_collection_index:
            ...
            if active_item.use_empty_origins:
                ...
                # Add note about automatic child following
                note_box = box.box()
                note_box.label(text="Note: Children automatically follow", icon='INFO')
                note_box.label(text="parent empties when centered")
