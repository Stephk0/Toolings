bl_info = {
    "name": "Mass Collection Exporter Fixed v8 - Proper Centering",
    "author": "Claude AI", 
    "version": (4, 6, 0),
    "blender": (3, 0, 0),
    "location": "3D View > N-Panel > Mass Exporter",
    "description": "Export collections with FIXED parent empty centering that handles parent transforms",
...
# FIXED CENTERING FUNCTIONS: Properly handle parent transforms
def move_empty_to_world_center(empty, debug_mode=False):
    """
...
def restore_empty_to_original_position(empty, original_world_pos, debug_mode=False):
    """
...
class MASSEXPORTER_OT_test_empty_centering(Operator):
    ...
    def test_empty_operations(self, empty_name, children_names, *, center_parent_empties=True, combine_empty_children=False):
        """Test centering and combining operations - USING FIXED CENTERING"""
        ...
        print(f"Original empty local: {empty.location}")
        print(f"Original empty world: {empty.matrix_world.translation}")
        if empty.parent:
        ...
        original_world_pos = None

        try:
            # Test 1: Center empty using FIXED APPROACH
            print("\n--- Testing Empty Centering (FIXED APPROACH) ---")
            if center_parent_empties:
                print("Moving empty to world center - children will follow automatically")
                
                # Use the fixed centering function
                original_world_pos = move_empty_to_world_center(empty, debug_mode=True)
            ...
            if empty and original_world_pos is not None:
                # Restore using FIXED APPROACH
                restore_empty_to_original_position(empty, original_world_pos, debug_mode=True)
                
                # Verify restoration
                final_pos = empty.matrix_world.translation
                restore_distance = (final_pos - original_world_pos).length
                if restore_distance > 0.001:
                    print(f"  ERROR: Failed to restore! Distance error: {restore_distance}")
                else:
                    print(f"  SUCCESS: Empty restored (error: {restore_distance})")
                ...
                for i, child_data in enumerate(children_original_world_positions):
                    ...
                    if child_obj:
                        ...
                        original_pos = child_data['world_position']
                        error = (current_world_pos - original_pos).length
                        if error > 0.001:
                            print(f"  {child_obj.name} WARNING: Not fully restored (error: {error})")
                        else:
                            print(f"  {child_obj.name} restored successfully")
            ...
            try:
                if original_world_pos is not None:
                    empty = bpy.data.objects.get(empty_name)
                    if empty:
                        restore_empty_to_original_position(empty, original_world_pos, debug_mode=True)
                bpy.ops.ed.undo()
...
class MASSEXPORTER_OT_export_all(Operator):
    ...
    def export_combined_empty_children(self, context, props, children, empty, export_path, item):
        """Combine children into single mesh and export - USING FIXED CENTERING"""
        if not children:
            return False
        ...
        try:
            # Step 1: Center empty if enabled - USING FIXED APPROACH
            if item.center_parent_empties:
                if props.debug_mode:
                    print("Moving empty to world center - children will follow")

                # Use the FIXED centering function
                original_world_pos = move_empty_to_world_center(empty, debug_mode=props.debug_mode)
            ...
            if empty and original_world_pos is not None:
                # Restore using FIXED APPROACH
                restore_empty_to_original_position(empty, original_world_pos, debug_mode=props.debug_mode)
    ...
    def export_objects_with_empty_origin(self, context, props, objects, empty, name, export_path, item):
        """Export objects using empty's location as origin - USING FIXED CENTERING"""
        if not objects:
            return False
        ...
        try:
            # Center parent empty if option is enabled - USING FIXED APPROACH
            if item.center_parent_empties:
                if props.debug_mode:
                    print("Moving empty to world center - children will follow")
    ...
    def draw(self, context):
        ...
        if props.collection_items and len(props.collection_items) > props.active_collection_index:
            ...
            if active_item.use_empty_origins:
                ...
                note_box = box.box()
                note_box.label(text="Fixed: Handles parent transforms", icon='INFO')
                note_box.label(text="Works with any hierarchy")
