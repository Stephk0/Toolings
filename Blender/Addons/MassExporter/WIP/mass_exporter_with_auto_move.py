bl_info = {
    "name": "Mass Collection Exporter - WITH AUTO MOVE TO ORIGIN",
    "author": "Claude AI", 
    "version": (5, 6, 0),
    "blender": (3, 0, 0),
    "location": "3D View > N-Panel > Mass Exporter",
    "description": "Export collections with option to auto-move empties to origin during export",
...
class CollectionExportItem(PropertyGroup):
    ...
    center_parent_empties: BoolProperty(
        ...
        description="EXACT BUTTON LOGIC: Move parent empty to (0,0,0), export, then restore position",
        default=True
    )

    move_empties_to_origin_on_export: BoolProperty(
        name="Move Empties to Origin on Export",
        description="Automatically run 'Move Empties to Origin' before export (uses exact same logic as the working button)",
        default=False
    )
...
class MASSEXPORTER_OT_move_empties_to_origin(Operator):
    ...
    def execute(self, context):
        """Move empties to origin - only those in enabled collections - THIS IS THE WORKING LOGIC"""
        return self.move_empties_to_origin_logic(context)
    
    def move_empties_to_origin_logic(self, context):
        """The core logic for moving empties to origin - extracted so it can be reused"""
        # Force OBJECT mode
        ...
        # Move each empty to origin - THIS IS THE WORKING LOGIC WE NEED TO COPY
        for empty_name, (empty, children) in unique_empties.items():
            ...
            # Move to origin - THIS IS THE WORKING PART
...
class MASSEXPORTER_OT_export_all(Operator):
    """Export all enabled collections with optional auto-move to origin"""
    bl_idname = "massexporter.export_all"
    bl_label = "Export All"
    bl_description = "Export all enabled collections with optional auto-move empties to origin"
    ...
    def execute(self, context):
        ...
        # If auto-move is enabled for any collection, run the move empties logic
        if auto_move_collections:
            if props.debug_mode:
                print(f"\n=== AUTO-MOVE TO ORIGIN ENABLED FOR {len(auto_move_collections)} COLLECTIONS ===")
            
            # Create a temporary move empties operator to use its logic
            move_op = MASSEXPORTER_OT_move_empties_to_origin()
            result = move_op.move_empties_to_origin_logic(context)
    ...
    def export_collection(self, context, props, item):
        ...
        if item.export_subcollections_as_single:
            ...
            return self.export_with_empty_origins_exact_button_logic(context, props, item)
        else:
    ...
    def export_with_empty_origins_exact_button_logic(self, context, props, item):
        """EXACT BUTTON LOGIC: Use exactly the same logic as the working 'move empties to origin' button"""
        collection = item.collection
        export_path = item.export_path
        success_count = 0

        if props.debug_mode:
            print(f"EXACT BUTTON LOGIC: Exporting with empty origins for: {collection.name}")

        # COPY THE EXACT LOGIC FROM THE WORKING BUTTON
        ...
        # Find empties with children in enabled collections - EXACT SAME FUNCTION
        def find_empties_in_collection(collection):
            """Recursively find empties with children in collection and sub-collections - COPIED FROM BUTTON"""
            empties_found = []
        ...
        # Collect all empties from enabled collections - EXACT SAME LOGIC
        all_empties_to_move = []
        for coll in enabled_collections:
            collection_empties = find_empties_in_collection(coll)
            all_empties_to_move.extend(collection_empties)
        
        # Remove duplicates (in case empty is in multiple collections) - EXACT SAME LOGIC
        unique_empties = {}
        for empty, children in all_empties_to_move:
            if empty.name not in unique_empties:
                unique_empties[empty.name] = (empty, children)
        ...
        # Process each empty using EXACT BUTTON LOGIC
        for empty_name, (empty, children) in unique_empties.items():
            if props.debug_mode:
            ...
            if item.center_parent_empties:
                ...
                try:
                    # EXACT BUTTON LOGIC: Move to origin
                    empty.location = (0.0, 0.0, 0.0)
                    bpy.context.view_layer.update()
                    ...
                    if item.combine_empty_children:
                        # Export combined
                        if self.export_children_combined_exact_logic(context, props, empty, children, export_path):
                            success_count += 1
                    else:
                        # Export individually
                        if self.export_children_individual_exact_logic(context, props, empty, children, export_path):
                            success_count += 1

                finally:
                    # EXACT BUTTON LOGIC: Restore position
    ...
    def export_children_combined_exact_logic(self, context, props, empty, children, export_path):
        """Export children combined - NO position manipulation, just export as-is"""
        if not children:
            return False
        ...
        if props.debug_mode:
            print(f"EXACT LOGIC: Combining children of {empty_name}")
        ...
        try:
            ...
            if props.debug_mode:
                print(f"Error in export_children_combined_exact_logic: {str(e)}")
            return False
    ...
    def export_children_individual_exact_logic(self, context, props, empty, children, export_path):
        """Export children individually - NO position manipulation, just export as-is"""
        if not children:
            return False
        ...
        if props.debug_mode:
            print(f"EXACT LOGIC: Exporting children of {empty_name} individually")
...
class MASSEXPORTER_PT_main_panel(Panel):
    ...
    bl_label = "Mass Exporter - WITH AUTO MOVE TO ORIGIN"
    ...
    def draw(self, context):
        ...
        # Info text
        centering_box.label(text="↑ THIS BUTTON WORKS PERFECTLY", icon='CHECKMARK')
        centering_box.label(text="New: Can auto-run this on export!", icon='LINKED')
        
        # Status info
        status_box = layout.box()
        status_box.label(text="NEW: Auto 'Move to Origin' option in collections", icon='CHECKMARK')
        status_box.label(text="Enable per collection in Parent Empty Options", icon='PIVOT_INDIVIDUAL')
    ...
    def draw(self, context):
        ...
        if props.collection_items and len(props.collection_items) > props.active_collection_index:
            ...
            if active_item.use_empty_origins:
                ...
                # NEW FEATURE: Auto move to origin option
                auto_box = box.box()
                ...
                if active_item.move_empties_to_origin_on_export:
                    ...
                    info_box.label(text="Will auto-run 'Move Empties to Origin'", icon='CHECKMARK')
                    info_box.label(text="before export using same logic as button", icon='LINKED')
                
                # Add note about EXACT BUTTON LOGIC
                note_box = box.box()
                note_box.label(text="EXACT BUTTON LOGIC: Same as working button!", icon='CHECKMARK')
                note_box.label(text="Move to (0,0,0) → Export → Restore", icon='LINKED')
