...
## Modifier List (Stephko fork) 1.9.89 - 17.4.2026

Fork of `Dangry98/Modifier_List_Fork` (`Modifier_List_Fork` 1.9.88).

Bug fixes:
- Geometry Nodes: the Input Attribute Toggle icon (spreadsheet) is now shown in list-view for all attribute-capable sockets, including Blender 5.x sockets whose `display_shape` is not a diamond (e.g. STRING inputs). Previously the icon only appeared in stack view because list-view relied solely on `display_shape` to detect attribute-capable sockets. List-view now additionally detects the `<socket>_use_attribute` IDProperty that Blender writes on the modifier, matching the signal Blender's own C-side `template_modifiers()` uses.
