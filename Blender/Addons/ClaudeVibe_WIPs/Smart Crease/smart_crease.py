...
bl_info = {
    ...
    "description": "Context-sensitive crease tool with modal control, quick presets (1-9, 0), and 0/1 toggle",
    "category": "Mesh",
    "doc_url": "https://www.artstation.com/stephko",
}
...
class MESH_OT_smart_crease(bpy.types.Operator):
    ...
    - C: Toggle between 0 and 1 (customizable in preferences)
    ...
    def invoke(self, context, event):
        ...
        self.toggle_key_type = None  # Will store the mapped event type
        ...
        if prefs:
            ...
            # Map toggle key string to Blender event type
            toggle_key = prefs.preferences.toggle_key.upper()
            if toggle_key and len(toggle_key) == 1:
                self.toggle_key_type = toggle_key
            else:
                self.toggle_key_type = 'C'  # Default fallback
        else:
            self.toggle_key_type = 'C'  # Default if no preferences
    ...
    def modal(self, context, event):
        ...
        # Toggle key (from preferences) - toggle 0/1
        elif event.type == self.toggle_key_type and event.value == 'PRESS' and not event.shift and not event.ctrl:
...
class SmartCreasePreferences(bpy.types.AddonPreferences):
    ...
    toggle_key: StringProperty(
        name="Toggle Key",
        description="Key to toggle between 0 and 1 (use single letter like C, V, T, etc.)",
        default="C",
        maxlen=1,
    )
    ...
    def draw(self, context):
        ...
        layout.prop(self, "toggle_key")
        layout.prop(self, "snap_increment")
        layout.prop(self, "hud_font_size")
