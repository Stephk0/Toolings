bl_info = {
    "name": "Mass Collection Exporter Fixed v7 - Simple Centering",
    "author": "Claude AI", 
    "version": (4, 5, 0),
    "blender": (3, 0, 0),
    "location": "3D View > N-Panel > Mass Exporter",
    "description": "Export collections with SIMPLE parent empty centering that actually works",
...
# SHARED FUNCTION: Simple world space empty centering
def move_empty_to_world_center(empty, debug_mode=False):
    """
    Move an empty to world center (0,0,0) using simple approach.
    ...
    offset = mathutils.Vector((0.0, 0.0, 0.0)) - original_world_pos
    
    # Move the empty by applying the offset to its current location
    empty.location = empty.location + offset
...
def restore_empty_to_original_position(empty, original_world_pos, debug_mode=False):
    """
    Restore an empty to its original world position.
    """
    if debug_mode:
        print(f"  Restoring {empty.name} to original world position {original_world_pos}")
    
    # Calculate offset from current world position to original
    current_world_pos = empty.matrix_world.translation
    offset = original_world_pos - current_world_pos
...
class MASSEXPORTER_OT_test_empty_centering(Operator):
    ...
    def test_empty_operations(self, empty_name, children_names, *, center_parent_empties=True, combine_empty_children=False):
        """Test centering and combining operations - SIMPLE APPROACH"""
...
class MASSEXPORTER_OT_export_all(Operator):
    ...
    def export_combined_empty_children(self, context, props, children, empty, export_path, item):
        """Combine children into single mesh and export - SIMPLE APPROACH"""
        if not children:
            return False
        ...
        try:
            # Step 1: Center empty if enabled - SIMPLE APPROACH
            if item.center_parent_empties:
                if props.debug_mode:
                    print("Moving empty to world center - children will follow")

                # Use the SAME simple centering function as the test
    ...
    def export_objects_with_empty_origin(self, context, props, objects, empty, name, export_path, item):
        """Export objects using empty's location as origin - SIMPLE APPROACH"""
        if not objects:
            return False
        ...
        try:
            # Center parent empty if option is enabled - SIMPLE APPROACH
            if item.center_parent_empties:
                if props.debug_mode:
                    print("Moving empty to world center - children will follow")
...
class MASSEXPORTER_PT_main_panel(Panel):
    ...
    bl_label = "Mass Exporter Fixed v7 - Simple Centering"
    ...
    def draw(self, context):
        ...
        if props.collection_items and len(props.collection_items) > props.active_collection_index:
            ...
            if active_item.use_empty_origins:
                ...
                note_box = box.box()
                note_box.label(text="Simple approach: Move to center and back", icon='INFO')
