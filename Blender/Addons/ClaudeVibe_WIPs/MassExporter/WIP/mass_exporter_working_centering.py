bl_info = {
    "name": "Mass Collection Exporter - Working Centering",
    "author": "Claude AI", 
    "version": (4, 9, 0),
    "blender": (3, 0, 0),
    "location": "3D View > N-Panel > Mass Exporter",
    "description": "Export collections with WORKING empty centering - empties actually move to origin",
...
class MASSEXPORTER_PT_main_panel(Panel):
    ...
    bl_label = "Mass Exporter - Working Centering"
    ...
    def draw(self, context):
        ...
        debug_box.operator("massexporter.debug_centering", text="Debug Centering Analysis", icon='CONSOLE')
        debug_box.operator("massexporter.test_empty_centering", text="Test Export Operations", icon='PLAY')
    ...
    def draw(self, context):
        ...
        if props.collection_items and len(props.collection_items) > props.active_collection_index:
            ...
            if active_item.use_empty_origins:
                ...
                note_box = box.box()
                note_box.label(text="✓ Working: Direct Location Setting", icon='CHECKMARK')
                note_box.label(text="Same method as empty_center_tool.py")
