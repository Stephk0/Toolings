bl_info = {
    "name": "Mass Collection Exporter - FIXED CENTERING",
    "author": "Claude AI", 
    "version": (5, 3, 0),
    "blender": (3, 0, 0),
    "location": "3D View > N-Panel > Mass Exporter",
    "description": "Export collections with working empty centering - FIXED CENTERING VERSION",
...
class CollectionExportItem(PropertyGroup):
    ...
    center_parent_empties: BoolProperty(
        ...
        description="Center parent empties and their children as groups during export",
        default=True
    )
...
class MASSEXPORTER_OT_export_all(Operator):
    ...
    def execute(self, context):
        ...
        # CRITICAL: Store original empty positions BEFORE any export operations
        ...
        try:
            ...
            # CRITICAL: Restore ALL empty positions at the very end
            if props.debug_mode:
    ...
    def export_collection(self, context, props, item):
        ...
        if item.export_subcollections_as_single:
            ...
            return self.export_with_empty_origins_fixed(context, props, item)
        else:
    ...
    def export_with_empty_origins_fixed(self, context, props, item):
        """FIXED: Export using empty objects as origins and group centers"""
        collection = item.collection
        export_path = item.export_path
        success_count = 0

        if props.debug_mode:
            print(f"FIXED: Exporting with empty origins for: {collection.name}")
        ...
        if empties_with_children:
            ...
            # Export each empty group with proper centering
            for empty, children in empties_with_children.items():
                ...
                if item.center_parent_empties:
                    # NEW FIXED METHOD: Calculate group center and export properly centered
                    if item.combine_empty_children:
                        if self.export_empty_group_combined_fixed(context, props, empty, children, export_path):
                            success_count += 1
                    else:
                        # Export each child individually, but all centered as a group
                        if self.export_empty_group_individual_fixed(context, props, empty, children, export_path):
                            success_count += 1
                else:
    ...
    def export_empty_group_combined_fixed(self, context, props, empty, children, export_path):
        """FIXED: Export empty's children as combined mesh, properly centered"""
        if not children:
            return False
        ...
        if props.debug_mode:
            print(f"FIXED: Combining and centering group {empty_name}")
            print(f"Empty at: {empty.location}")
            print(f"Children: {[child.name for child in children]}")
        ...
        for child in children:
            original_child_positions.append({
                'obj': child,
        ...
        try:
            # STEP 1: Calculate the group's bounding box center
            group_center = self.calculate_group_center(children)
            
            if props.debug_mode:
                print(f"Group center calculated as: {group_center}")

            # STEP 2: Move empty to origin and adjust children to maintain relative positions
            empty.location = (0.0, 0.0, 0.0)
            
            # Calculate offset needed to center the group
            center_offset = mathutils.Vector(group_center) * -1
            
            if props.debug_mode:
                print(f"Center offset: {center_offset}")

            # Apply offset to all children to center the group
            for child in children:
                child.location = child.location + center_offset
                if props.debug_mode:
            ...
            if len(children) > 1:
                ...
                if props.debug_mode:
                    print(f"Combined object at: {combined_obj.location}")
            ...
            if props.debug_mode:
                print(f"Error in export_empty_group_combined_fixed: {str(e)}")
            return False

        finally:
            # Restore original positions
            empty.location = original_empty_pos
            for orig_data in original_child_positions:
                orig_data['obj'].location = orig_data['location']
                orig_data['obj'].rotation_euler = orig_data['rotation']
                orig_data['obj'].scale = orig_data['scale']
    ...
    def export_empty_group_individual_fixed(self, context, props, empty, children, export_path):
        """FIXED: Export empty's children individually, all properly centered as a group"""
        if not children:
            return False
        ...
        if props.debug_mode:
            print(f"FIXED: Exporting group {empty_name} individually, centered")
        ...
        try:
            # STEP 1: Calculate the group's center
            group_center = self.calculate_group_center(children)
            
            if props.debug_mode:
                print(f"Group center: {group_center}")

            # STEP 2: Move empty to origin
            empty.location = (0.0, 0.0, 0.0)
            
            # Calculate offset to center the group
            center_offset = mathutils.Vector(group_center) * -1

            # STEP 3: Export each child individually with the group properly centered
            for i, child in enumerate(children):
                # Apply centering offset to this child
                child.location = child.location + center_offset
                bpy.context.view_layer.update()

                if props.debug_mode:
                    print(f"Exporting {child.name} at centered position: {child.location}")
                ...
                # Restore child's original position for next iteration
                child.location = original_child_positions[i]['location']

            return success_count > 0

        except Exception as e:
            if props.debug_mode:
                print(f"Error in export_empty_group_individual_fixed: {str(e)}")
            return False
    ...
    def calculate_group_center(self, objects):
        """Calculate the center point of a group of objects"""
        if not objects:
...
class MASSEXPORTER_PT_main_panel(Panel):
    ...
    bl_label = "Mass Exporter - FIXED CENTERING"
    ...
    def draw(self, context):
        ...
        status_box.label(text="FIXED: Proper group centering in export", icon='CHECKMARK')
        status_box.label(text="Groups exported with empty/pivot at world center", icon='PIVOT_INDIVIDUAL')
    ...
    def draw(self, context):
        ...
        if props.collection_items and len(props.collection_items) > props.active_collection_index:
            ...
            if active_item.use_empty_origins:
                ...
                # Add note about FIXED centering
                note_box = box.box()
                note_box.label(text="FIXED: Proper group centering", icon='CHECKMARK')
                note_box.label(text="Groups exported with pivot at world center")
