bl_info = {
    "name": "Mass Collection Exporter Enhanced v2",
    "author": "Claude AI",
    "version": (2, 0, 0),
    "blender": (3, 0, 0),
    "location": "3D View > N-Panel > Mass Exporter",
    "description": "Export collections with sub-collection and empty-origin support",
...
class MASSEXPORTER_OT_export_all(Operator):
        ...
        if empties_with_children:
            ...
            for empty, children in empties_with_children.items():
                if item.combine_empty_children:
                    ...
                    if self.export_objects_with_empty_origin(context, props, children, empty, f"{empty.name}_combined", export_path):
                        success_count += 1
                else:
                    # Export each child individually but with empty as origin
                    for child in children:
                        if self.export_objects_with_empty_origin(context, props, [child], empty, child.name, export_path):
                            success_count += 1
    ...
    def export_objects_with_empty_origin(self, context, props, objects, empty, name, export_path):
        """Export objects using empty's location as origin"""
        if not objects:
            return False
        ...
        empty_location = empty.location.copy()
        empty_rotation = empty.rotation_euler.copy()
        empty_scale = empty.scale.copy()
        
        try:
            # Move empty to origin and adjust children accordingly
            offset = -empty.location
            empty.location = (0, 0, 0)
            
            # Move all objects relative to empty's new position
            for obj in objects:
            ...
            if not props.apply_transforms:
                ...
                empty.rotation_euler = empty_rotation
                empty.scale = empty_scale
...
class MASSEXPORTER_PT_main_panel(Panel):
    ...
    bl_label = "Mass Exporter Enhanced v2"
    ...
    def draw(self, context):
        ...
        if props.collection_items and len(props.collection_items) > props.active_collection_index:
            ...
            if active_item.use_empty_origins:
                layout.prop(active_item, "combine_empty_children")
