bl_info = {
    "name": "Mass Collection Exporter Fixed v5 - World Space Fix",
    "author": "Claude AI",
    "version": (4, 3, 0),
    "blender": (3, 0, 0),
    "location": "3D View > N-Panel > Mass Exporter",
    "description": "Export collections with PROPERLY FIXED world-space parent empty centering",
...
class CollectionExportItem(PropertyGroup):
    ...
    center_parent_empties: BoolProperty(
        ...
        description="Move parent empties to world center (0,0,0) before export - uses world space positioning",
        default=True
    )
...
class MASSEXPORTER_OT_test_empty_centering(Operator):
    ...
    def test_empty_operations(self, empty_name, children_names, *, center_parent_empties=True, combine_empty_children=False):
        """Test centering and combining operations - FIXED to use world space transforms"""
        ...
        print(f"Original empty local location: {empty.location}")
        print(f"Original empty world location: {empty.matrix_world.translation}")
        print(f"Children: {children_names}")

        # Store original world matrix - FIXED
        ...
        for child in children:
            # Get world matrix to capture the actual world position
        ...
        try:
            # Test 1: Center empty to world origin - FIXED
            print("\n--- Testing Empty Centering (WORLD SPACE FIXED) ---")
            if center_parent_empties:
                print(f"Moving empty from world {empty.matrix_world.translation} to world (0,0,0)")
                print("Children will automatically follow due to parent-child relationship")
                
                # Set world translation to origin - FIXED
                empty.matrix_world.translation = mathutils.Vector((0.0, 0.0, 0.0))
                
                # Update the scene to reflect changes
                bpy.context.view_layer.update()
                
                print(f"Empty now at world: {empty.matrix_world.translation}")
                print(f"Empty local position: {empty.location}")
            ...
            if combine_empty_children and len(children) > 1:
                ...
                # DON'T access object references after join - they may be invalid
            ...
            if empty:
                # Restore world matrix - FIXED
                print(f"Restoring empty to original world matrix")
                empty.matrix_world = empty_world_matrix
                bpy.context.view_layer.update()
                
                print(f"Empty restored to world: {empty.matrix_world.translation}")
                print(f"Empty local position: {empty.location}")
...
class MASSEXPORTER_OT_export_all(Operator):
    ...
    def export_combined_empty_children(self, context, props, children, empty, export_path, item):
        """Combine children into single mesh and export with proper world-space centering"""
        if not children:
            return False
        ...
        if props.debug_mode:
            print(f"\n--- Combining and exporting children of {empty_name} ---")
        ...
        # Store original world matrix for restoration - FIXED
        empty_world_matrix = empty.matrix_world.copy()

        try:
            # Step 1: Center empty if enabled - FIXED VERSION
            if item.center_parent_empties:
                if props.debug_mode:
                    print(f"Centering empty from world {empty.matrix_world.translation} to world (0,0,0)")
    ...
    def export_objects_with_empty_origin(self, context, props, objects, empty, name, export_path, item):
        """Export objects using empty's location as origin with proper world-space centering"""
        if not objects:
            return False
        ...
        # Store empty's original world matrix - FIXED
        empty_world_matrix = empty.matrix_world.copy()

        try:
            # Center parent empty if option is enabled - FIXED VERSION
            if item.center_parent_empties:
                if props.debug_mode:
                    print(f"Centering empty from world {empty.matrix_world.translation} to world (0,0,0)")
                    print("Children will automatically follow the parent")
            ...
            # Always restore original world matrix - FIXED
            if props.debug_mode:
                print("Restoring original world matrix...")

            # Get fresh reference to empty
            empty = bpy.data.objects.get(empty_name)
            if empty:
                # Restore empty's original world matrix - FIXED
                empty.matrix_world = empty_world_matrix
                bpy.context.view_layer.update()
...
class MASSEXPORTER_PT_main_panel(Panel):
    ...
    bl_label = "Mass Exporter Fixed v5 - World Space"
    ...
    def draw(self, context):
        ...
        if props.collection_items and len(props.collection_items) > props.active_collection_index:
            ...
            if active_item.use_empty_origins:
                ...
                # Add note about world space positioning
                note_box = box.box()
                note_box.label(text="Note: Uses world space positioning", icon='INFO')
                note_box.label(text="Works correctly with parented empties")
