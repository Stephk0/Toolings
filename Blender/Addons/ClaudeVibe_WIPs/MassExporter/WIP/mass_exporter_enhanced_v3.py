bl_info = {
    "name": "Mass Collection Exporter Enhanced v3",
    "author": "Claude AI",
    "version": (3, 0, 0),
    "blender": (3, 0, 0),
    "location": "3D View > N-Panel > Mass Exporter",
    "description": "Export collections with enhanced parent empty centering and sub-collection support",
...
class CollectionExportItem(PropertyGroup):
    ...
    combine_empty_children: BoolProperty(
        ...
        description="Combine objects that are children of empties into single objects",
        default=False
    )
...
class MASSEXPORTER_OT_test_empty_centering(Operator):
    """Test parent empty centering functionality"""
    bl_idname = "massexporter.test_empty_centering"
    bl_label = "Test Empty Centering"
    bl_description = "Test the parent empty centering functionality without exporting"
    
    def execute(self, context):
        props = context.scene.mass_exporter_props
        
        if not props.collection_items:
            self.report({'WARNING'}, "No collections to test")
            return {'CANCELLED'}
        
        # Find an active collection with empty origins enabled
        test_item = None
        for item in props.collection_items:
            if item.export_enabled and item.use_empty_origins and item.collection:
                test_item = item
                break
        
        if not test_item:
            self.report({'WARNING'}, "No enabled collections with 'Use Parent Empties' found")
            return {'CANCELLED'}
        
        # Test empty centering
        empties_with_children = self.find_empties_with_children(test_item.collection)
        
        if not empties_with_children:
            self.report({'WARNING'}, f"No parent empties found in collection: {test_item.collection.name}")
            return {'CANCELLED'}
        
        # Demonstrate centering for first empty found
        empty, children = next(iter(empties_with_children.items()))
        self.demonstrate_empty_centering(empty, children, test_item)
        
        self.report({'INFO'}, f"Tested centering for empty: {empty.name}")
        return {'FINISHED'}
    ...
    def demonstrate_empty_centering(self, empty, children, item):
        """Demonstrate the empty centering process"""
        print(f"\n=== TESTING EMPTY CENTERING FOR: {empty.name} ===")
        ...
        children_original_locations = [child.location.copy() for child in children]
        
        # Center the empty and adjust children
        ...
        for child, orig_loc in zip(children, children_original_locations):
...
class MASSEXPORTER_OT_export_all(Operator):
        ...
        if empties_with_children:
            ...
            for empty, children in empties_with_children.items():
                ...
                if item.combine_empty_children:
                    # Combine all children into single export
                    if self.export_objects_with_empty_origin(context, props, children, empty, f"{empty.name}_combined", export_path, item):
                        success_count += 1
                else:
    ...
    def export_objects_with_empty_origin(self, context, props, objects, empty, name, export_path, item):
        """Export objects using empty's location as origin with enhanced centering"""
        if not objects:
            return False
...
class MASSEXPORTER_PT_main_panel(Panel):
    ...
    bl_label = "Mass Exporter Enhanced v3"
    ...
    def draw(self, context):
        ...
        # Test button
        layout.operator("massexporter.test_empty_centering", text="Test Empty Centering", icon='EMPTY_ARROWS')
