...
def NODES(self, layout, ob, md):
    ...
    try:
        from . import (get_geonode_input_sockets, is_drivable_socket_type,
                       is_reference_socket_type)
    except ImportError:
        layout.label(text="Geometry Nodes support unavailable", icon='ERROR')
        return
    ...
    if md.node_group.library:
        row.label(text=f"{md.node_group.name} (Linked)", icon='LINKED')
    else:
        row.label(text=f"{md.node_group.name}", icon='NODETREE')
    row.prop(md, "node_group", text="")
    ...
    for socket_info in sockets:
        ...
        if is_reference_socket_type(bl_socket_idname):
            ...
            split.prop(md, f'["{identifier}"]', text=name)

            # Add sync button for reference types
            op = split.operator("rtools.sync_geonode_input", text="Sync")
            op.mod = md.name
            op.object = ob.name
            op.identifier = identifier
            op.socket_type = bl_socket_idname

            # Show driver indicator if has driver (for tracking)
            if has_driver:
                split.label(text="", icon='INFO')
