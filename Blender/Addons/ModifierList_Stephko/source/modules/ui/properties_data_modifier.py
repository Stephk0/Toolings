    ...
    def _nodes_4_0_inputs(self, layout, ob, md, split_facor):
        ...
        def get_socket_prop_id(input_info):
                ...
                if input_type in datablock_input_info_per_type.keys():
                    ...
                    # A socket is attribute-capable if either its display shape is a diamond
                    # (legacy float/int/bool/vector/color pipeline) or the modifier exposes a
                    # `<prop_id>_use_attribute` IDProperty. The latter is the definitive signal
                    # from Blender and is needed for newer socket types (e.g. STRING in 5.x)
                    # whose display_shape isn't a diamond even though they can toggle between
                    # a value and an attribute. Without this fallback, the Input Attribute
                    # Toggle icon was missing in list-view for those sockets, while stack view
                    # (Blender's native template_modifiers) still rendered it correctly.
                    # Menus are still excluded to match Blender's built-in behaviour and to
                    # avoid the crash that originally motivated issue #44.
                    use_attr_key = f"{prop_id}_use_attribute"
                    is_attribute_capable = (
                        input_info["accepts_attribute"] or use_attr_key in md.keys()
                    )
                    if is_attribute_capable and input_type != 'MENU':
                        if md[use_attr_key] == 1:
